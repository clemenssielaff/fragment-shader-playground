...
void main(){
    gl_FragColor = vec4(time,tan(time),cos(time),1.0);  
}
`;
...
function linearInterpolate(value, inputRange, outputRange) {
	...
	const res = (1-t) * outputRange[0] + t * outputRange[1];
	return res;
}
/**
 * @param now The current time in seconds
 * @param startTime The time when the interval shall start in s
 * @param animationTime The time needed to do the animation in s
 * @param waitTime The time to wait before the next animation sequence in s
 * @param rotationAmount The amount to rotate in each animation squence in RAD
 */
function rotateInInterval(now, startTime, animationTime, waitTime, rotationAmount){
	const _time = now - startTime
    if(_time >= 0) {
        const sequenceDuration = (animationTime + waitTime)
        const block = Math.floor(_time / sequenceDuration)
        let totalRotation = block*rotationAmount
        if (_time - block*(sequenceDuration) <= animationTime) {
            totalRotation += linearInterpolate(_time - block*(sequenceDuration),[0,animationTime],[0, rotationAmount])
        } else {
...
function drawScene(gl, programInfo, deltaTime, time, now, last) {
    ...
    mat4.rotate(
        ...
        Math.PI * 0.25 + rotateInInterval(time, 0, 1, 2, 0.5*Math.PI),             // amount to rotate in radians    
        [0, 0, 1]);         // axis to rotate around (Z)
    mat4.rotate(
        modelViewMatrix,    // destination matrix
        modelViewMatrix,    // matrix to rotate
        Math.PI * 0.25 + rotateInInterval(time, 1, 1, 2, 0.5*Math.PI),                // amount to rotate in radians
        [0, 1, 0]);         // axis to rotate around (Y)
    mat4.rotate(
        modelViewMatrix,    // destination matrix
        modelViewMatrix,    // matrix to rotate
        Math.PI * 0.25 + rotateInInterval(time, 2, 1, 2, 0.5*Math.PI),                // amount to rotate in radians
    ...
    gl.uniform1f(
        programInfo.uniformLocations.deltaTime,time - deltaTime % now
    );
